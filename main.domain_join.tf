...
data "azurerm_key_vault_secret" "domain_join_user_password" {
  count        = var.domain_join_user_key_vault_resource_id != null && var.domain_join_user_key_vault_secret_name != null ? 1 : 0
  name         = var.domain_join_user_key_vault_secret_name
  key_vault_id = var.domain_join_user_key_vault_resource_id
}
