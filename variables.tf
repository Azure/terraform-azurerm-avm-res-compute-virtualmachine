...
SHUTDOWN_SCHEDULES
  ...
  validation {
    condition     = length(var.shutdown_schedules) <= 1
    error_message = "Only one shutdown schedule can be defined per virtual machine. Azure derives the schedule's name from the machine's name, so every entry in this map resolves to the same Azure resource."
  }
}
...
variable "source_image_resource_id" {
  ...
  default     = null
  description = <<-DESCRIPTION
  The Azure resource ID of the source image used to create the VM. Either `source_image_resource_id` or `source_image_reference` must be set and both can not be null at the same time. Not used when `os_managed_disk_id` is set.
