...
resource "azurerm_virtual_machine_extension" "this_extension_3" {
  ...
  for_each = toset([for k, v in nonsensitive(var.extensions) : k if v.deploy_sequence == 3])
...
resource "azurerm_virtual_machine_extension" "this_extension_4" {
  #forcing to use the map key to address terraform limitation around sensitive values in the map (https://developer.hashicorp.com/terraform/language/meta-arguments/for_each#limitations-on-values-used-in-for_each)
  for_each = toset([for k, v in nonsensitive(var.extensions) : k if v.deploy_sequence == 4])
